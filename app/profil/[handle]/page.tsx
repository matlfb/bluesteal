...
export default function ProfilPage() {
  ...
  useEffect(() => {
    ...
    return (
      <div>
        {/* Banner */}
        <div style={{ height: 200, background: 'var(--surface)', animation: 'skeleton 1.4s ease-in-out infinite' }} />
        {/* Header */}
        <div style={{ borderBottom: '1px solid rgba(0,229,255,0.08)' }}>
          <div className="profile-header-wrap">
            <div className="profile-header-row">
              <div style={{ width: 112, height: 112, flexShrink: 0, background: 'var(--elevated)', animation: 'skeleton 1.4s ease-in-out infinite', border: '3px solid var(--bg)' }} />
              <div className="profile-info">
                <div className="profile-meta">
                  <div>
                    <div style={{ height: 38, width: 220, background: 'var(--surface)', animation: 'skeleton 1.4s ease-in-out infinite', marginBottom: 10 }} />
                    <div style={{ height: 14, width: 130, background: 'var(--surface)', animation: 'skeleton 1.4s ease-in-out infinite' }} />
                  </div>
                  <div className="profile-actions">
                    <div style={{ height: 38, width: 180, background: 'var(--surface)', animation: 'skeleton 1.4s ease-in-out infinite' }} />
                  </div>
                </div>
                <div style={{ height: 14, maxWidth: 460, width: '100%', background: 'var(--surface)', animation: 'skeleton 1.4s ease-in-out infinite', marginTop: 16 }} />
                <div style={{ height: 14, maxWidth: 320, width: '80%', background: 'var(--surface)', animation: 'skeleton 1.4s ease-in-out infinite', marginTop: 8 }} />
                <div className="profile-stats">
                  {[80, 80, 80].map((_, i) => (
                    <div key={i}>
                      <div style={{ height: 8, width: 60, background: 'var(--surface)', animation: 'skeleton 1.4s ease-in-out infinite', marginBottom: 10 }} />
                      <div style={{ height: 20, width: 48, background: 'var(--surface)', animation: 'skeleton 1.4s ease-in-out infinite' }} />
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="profile-tabs-section">
          <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid rgba(0,229,255,0.08)', marginBottom: '2rem' }}>
            {[80, 80, 80].map((_, i) => (
              <div key={i} style={{ height: 36, width: 100, background: 'var(--surface)', animation: 'skeleton 1.4s ease-in-out infinite', margin: '0 4px 0 0' }} />
            ))}
          </div>
          {/* Collection grid */}
          <div className="collection-grid">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} style={{ aspectRatio: '4/5', background: 'var(--surface)', animation: 'skeleton 1.4s ease-in-out infinite' }} />
            ))}
          </div>
        </div>
      </div>

...
function LoginForm() {
  ...
  const [error, setError] = useState<string | null>(
    searchParams.get('blocked') === '1' ? "Unable to sign in to BlueSTEAL. Please make sure you have not blocked @bluesteal.app on Bluesky." : null
  )
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (check.blocked) {
        setError("Unable to sign in to BlueSTEAL. Please make sure you have not blocked @bluesteal.app on Bluesky.")
        setLoading(false)
        return
      }

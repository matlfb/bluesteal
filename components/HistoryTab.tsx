...
export default function HistoryTab({ events, cardHistory }: { events: HistoryEvent[], cardHistory: HistoryEvent[] }) {
  const { t, fmtDate, fmtNum } = useLang()
  const [segment, setSegment] = useState<'transactions' | 'card'>('transactions')
  const activeEvents = segment === 'transactions' ? events : cardHistory

  const segBtnStyle = (active: boolean) => ({
    fontFamily: 'var(--font-mono)', fontSize: '10px', fontWeight: 600,
    letterSpacing: '0.1em', padding: '5px 14px', cursor: 'pointer', border: 'none',
  ...
  return (
    <div style={{ maxWidth: 1020 }}>
      <div style={{ display: 'flex', gap: 0, marginBottom: '1.5rem', borderBottom: '1px solid rgba(0,229,255,0.08)' }}>
        <button style={segBtnStyle(segment === 'transactions')} onClick={() => setSegment('transactions')}>
          TRANSACTIONS
        </button>
        <button style={segBtnStyle(segment === 'card')} onClick={() => setSegment('card')}>
          CARD HISTORY
        </button>
      </div>
      ...
      <div style={{ borderTop: activeEvents.length > 0 ? '1px solid rgba(0,229,255,0.08)' : 'none' }}>
        {activeEvents.map((ev, i) => {
          const isBought = ev.type === 'bought'
          const isCardView = segment === 'card'
          return (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '90px 1fr auto', alignItems: 'start', gap: '1.5rem', padding: '1.25rem 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
              ...
              <div>
                {isCardView ? (
                  // Card history: show who bought this card
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.3rem' }}>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', fontWeight: 600, letterSpacing: '0.12em', padding: '2px 7px', background: 'rgba(0,229,255,0.1)', color: 'var(--brand)', border: '1px solid rgba(0,229,255,0.2)' }}>
                        ACQUIRED
                      </span>
                      {ev.actor_avatar && (
                        <img src={ev.actor_avatar} alt="" width={18} height={18} style={{ borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
                      )}
                      {ev.actor_handle ? (
                        <Link href={'/profil/' + ev.actor_handle} style={{ fontFamily: 'var(--font-mono)', fontSize: '13px', color: 'var(--t1)', textDecoration: 'none', fontWeight: 500 }}
                          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = 'var(--brand)' }}
                          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = 'var(--t1)' }}>
                          @{ev.actor_handle}
                        </Link>
                      ) : (
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '13px', color: 'var(--t3)' }}>{ev.actor_did.slice(0, 20)}…</span>
                      )}
                    </div>
                  ...
                  // Transactions: show what the user bought/lost
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.3rem' }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', fontWeight: 500, color: isCardView ? 'var(--t2)' : (isBought ? 'var(--danger)' : 'var(--success)'), paddingTop: 3, whiteSpace: 'nowrap' }}>
                {!isCardView && (isBought ? '-' : '+')}{fmtNum(ev.price)} J
              </span>
            </div>
          )
        })}
